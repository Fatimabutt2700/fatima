...
stream = cv2.VideoCapture("desion pendings.mp4")
flag = True
def play(speed):
    global flag
...
def pending(decision):
    ...
    frame = cv2.cvtColor(cv2.imread("desion.png"), cv2.COLOR_BGR2RGB)
    frame = imutils.resize(frame, width=SET_WIDTH, height=SET_HEIGHT)
    frame = PIL.ImageTk.PhotoImage(image=PIL.Image.fromarray(frame))
    canvas.image = frame
    canvas.create_image(0,0, image=frame, anchor=tkinter.NW)
...
cv_img = cv2.cvtColor(cv2.imread("download1.png"), cv2.COLOR_BGR2RGB)
